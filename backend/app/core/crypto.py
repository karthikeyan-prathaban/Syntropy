...
def _get_encryption_key() -> bytes:
    ...
    raw = settings.encryption_key or settings.jwt_secret or "novaa-secure-default-salt-2026"
    salt = b"novaa-fiu-aes256-rebit-compliance-salt"
    return hashlib.sha256(raw.encode("utf-8") + salt).digest()


def encrypt_field(plain_text: Optional[str]) -> str:
...
def decrypt_field(cipher_text: Optional[str]) -> str:
