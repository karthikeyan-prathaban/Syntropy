...
def parse_deposit_fi_data(fi_payload: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for fip in fi_payload.get("fips") or []:
        ...
        for account_block in fip.get("accounts") or []:
            ...
            deposit = (
                account_block.get("deposit")
                or account_block.get("DEPOSIT")
                or data_field.get("deposit")
                or data_field.get("DEPOSIT")
                or data_field
                or account_block
            )
            ...
            if not account and isinstance(deposit, dict) and (
                "maskedAccNumber" in deposit or "masked_acc_number" in deposit
            ):
                account = deposit
            if not account and isinstance(account_block, dict) and (
                "maskedAccNumber" in account_block or "linkRefNumber" in account_block
            ):
            ...
            txns_list = (
                txns_block.get("transaction") or txns_block.get("transactions") or []
            ...
            for txn in txns_list:
                ...
                bal_after = txn.get("currentBalance") or txn.get("balanceAfter")
                try:
                ...
                transactions.append(
                    {
                        ...
                        or txn.get("txn_id")
                        or f"{linked_ref}-{len(transactions)}",
                        "linked_acc_ref": linked_ref,
                        "amount": amount,
                        "txn_type": txn_type,
                        "narration": narration,
                        ...
                        "transaction_timestamp": _parse_ts(
                            txn.get("transactionTimestamp") or txn.get("transaction_timestamp")
                        ),
